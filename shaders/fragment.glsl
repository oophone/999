...
void main() {
    ...
    vec3 refractDir = normalize(v_refract + (vec3(0.0, 0.0, 1.0) - N.xyz));
    ...
    vec3 refractDir = normalize(v_refract);
	#endif
	#endif
...
vec3 environment2 = textureCube(u_environmentCubemap, refractDir).rgb;
...
vec3 glasscol=mix(environment,environment2,u_fes.z);
	
	specular *= glasscol;
	#ifdef reflectionColorFlag
	diffuse.rgb = saturate(vec3(1.0) - u_reflectionColor.rgb) * diffuse.rgb + glasscol * u_reflectionColor.rgb;
	#endif
	#endif
